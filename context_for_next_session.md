...
このドキュメントは **2026/03/21 セッション（全作業完了時点）** でのプロジェクトの全情報を統合したものです。
...
#### ⑬ ダッシュボード UI/UX 改善（サークル中心レイアウト）

ダッシュボードのレイアウトを「サークル中心」に全面刷新。情報の優先度を見直し、サークルを主役に。

- `lib/actions/dashboard.actions.ts` — `CircleDetail` 型を追加。各サークルのメンバー数・次のイベント情報を一括取得する `circleDetails` を返すよう拡張
- `app/(dashboard)/dashboard/page.tsx` — レイアウトを全面改修：
  - あいさつを大きなヘッダー2行 → コンパクトな1行（名前 + 日付）に変更
  - やることリストは 0件のとき非表示（「✅ やることはありません」の空表示を廃止）
  - サークル一覧をページ最下部の小さなタグ → **主役のカード形式**に変更
  - 各カードに「👥 メンバー数 | 📅 次のイベント」を常に表示（イベントなしの場合は「予定なし」）
  - アイコンを角丸背景付き枠で表示
  - 「今後のイベント」独立セクションを削除し、サークルカード内に統合
- `components/home/UpcomingEvents.tsx` — 引き続き存在（他ページからの参照可能性あり）

#### ⑫ カスタムアイコン統合（前回セッション 2026/03/20）

テキスト絵文字を全てカスタム画像アイコンに置き換え。

- `lib/utils/circleIcon.ts`（新規）— 絵文字→画像パスの変換ユーティリティ（`resolveCircleIcon`）
- `components/circles/CircleIcon.tsx`（新規）— 共通アイコン表示コンポーネント
- `public/assets/circle-icons/` — 16種のカスタムアイコン画像
- ダッシュボード、サイドバー、各ページでテキスト絵文字→ `CircleIcon` に置き換え
